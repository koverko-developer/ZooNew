...
public class MainActivity extends AppCompatActivity
    ...
    private void setRecyclerRecomended(){
        List<RecomendedProductHour> list = new ArrayList<>();

        RecyclerView recyclerView = (RecyclerView) findViewById(
                R.id.recyclerRecomended);
        //recyclerView.addItemDecoration(new MarginDecoration(this));
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new GridLayoutManager(this, 2));
        for(int i = 0; i< 10; i++){
            RecomendedProductHour product = new RecomendedProductHour();
            product.setName("Product"+i);
            product.setPrice(String.valueOf(100+i) + " Br");
            product.setEndPrice(String.valueOf(i+10)+" Br");
            product.setTime("3600000");
            product.setTine(false);
